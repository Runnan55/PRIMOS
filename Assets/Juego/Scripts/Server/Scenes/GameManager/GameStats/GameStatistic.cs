...
public class GameStatistic : NetworkBehaviour
{
    ...
    public void UpdatePlayerStats(PlayerController player, bool disconnected = false)
    {
        ...
        if (gameManager != null)
        {
            ...
            // Si hay MatchHandler con modo, �salo como fuente de verdad
        ...
        // --- Total de jugadores para calcular posici�n ---
        // (Durante el cierre se habr� rellenado toda la lista; en updates intermedios,
        // players.Count puede ser menor, pero al final se corrige con el snapshot final.)
        int totalPlayers = Mathf.Max(1, players.Count);

        // --- Posici�n desde deathOrder ---
        // deathOrder: 1 = primer muerto ... N = ganador (�ltimo en morir/asignarse)
    ...
    public void ShowLeaderboard()
    {
        ...
        // antes hac�a: players.OrderByDescending(p => p.deathOrder)
